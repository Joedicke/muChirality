...
def plot_results_mesh_refinement():
    ...
    axes[2].plot(nb_dof_muspectre_all, time_muspectre, color=color_mu,
                 marker='x', label=r'$\mathrm{\mu}$Spectre')

    axes[2].text(1.5e5, 1e3, 'COMSOL', color=color_com, fontsize=fontsize_small)
    axes[2].text(5e6, 1e1, r'$\mathrm{\mu}$Spectre', color=color_mu, fontsize=fontsize_small)
    ...
    # Number subplots
    axes[0].text(2.6e6, 0.01184, '(a) Convergence COMSOL', fontsize=fontsize, va='bottom', ha='center', weight='bold')
    axes[1].text(180, 0.00776, r'(b) Convergence $\mathbf{\mu}$Spectre', fontsize=fontsize, ha='center', weight='bold')
    axes[2].text(4e4, 5.5e3, '(c) Calculation time', fontsize=fontsize, weight='bold', ha='left')

    ### ----- Finish ----- ###
    name = 'results_paper/chi_chiral_mesh_refinement.pdf'
    fig.savefig(name, bbox_inches='tight')
    plt.show()
    plt.close(fig)

##################################################################
### ------------- Plot force vs. parameters ------------- ###
##################################################################
def plot_force_vs_unit_cells():
    ...
    ax.text(1.0, 7.7e-3, r'$\mathrm{\mu}$Spectre: $N_{x}$=$N_{y}$=1', fontsize=fontsize, color=color_1x1)
    ax.text(1.0, 2.6e-3, r'$\mathrm{\mu}$Spectre: $N_{x}$=$N_{y}$=3', fontsize=fontsize, color=color_3x3)

    ### ----- Finish ----- ###
    name = 'results_paper/chi_chiral_force_vs_unit_cells.pdf'
    fig.savefig(name, bbox_inches='tight')
    plt.show()
    plt.close(fig)
...
def plot_force_vs_twist():
    ...
    first_legend = ax.legend(handles=[line_mu_cyl, line_mu_1, line_mu_3], loc='upper left',
                                 fontsize=fontsize, title=r'$\mathrm{\mu}$Spectre', title_fontsize=fontsize+2,
                                 labelspacing=0.2)
    ax.add_artist(first_legend)
    ax.legend(handles=[line_com_1, line_com_3], loc='lower right', fontsize=fontsize, labelspacing=0.2,
              title=r'COMSOL', title_fontsize=fontsize+2)
    ...
    ### ----- Finish ----- ###
    name = 'results_paper/chi_chiral_force_vs_twist.pdf'
    fig.savefig(name, bbox_inches='tight')
    plt.show()
    plt.close(fig)

##################################################################
### ------------- Plot 2D stress chiral metamat -------------- ###
##################################################################
